...
def create_fixed_loans_df(lrfile_path, monthlyDF, maindflen = 81689):
    ...
    # Support CSV or Excel for LoanAsAtDate files
    if isinstance(lrfile_path, str) and lrfile_path.lower().endswith('.csv'):
        lr_df = pd.read_csv(lrfile_path, skiprows=3)
    else:
    ...
    fixed_loans_df['Client No'] = np.nan
